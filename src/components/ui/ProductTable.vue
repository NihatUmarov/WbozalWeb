<template>
  ...
  >
    ...
    <template #cell(cArt)="{ item }">
      <AppTableCell :value="item.cArt" mono bold align="center" bg="secondary" border :px="6" :py="4" />
    </template>
    <template #cell(cArtWB)="{ item }">
      <AppTableCell :value="item.cArtWB" mono bold align="center" bg="secondary" border :px="6" :py="4" />
    </template>
    ...
    <template #cell(cName)="{ item }">
      <div class="flex flex-col min-w-0">
        <div class="flex items-center gap-6">
          <span class="text-sm font-semibold text-primary truncate" :title="String(item.cName || '')">
            {{ item.cName || '—' }}
          </span>
    ...
    <template #cell(size)="{ item }">
      <AppTableCell :value="item.size" bold align="center" />
    </template>

    <!-- Barcodes -->
    <template #cell(barcodes)="{ item }">
      <div v-if="item.barcodes?.length" class="barcodes-wrap-list min-w-0 max-h-full overflow-hidden justify-center items-center py-4">
    ...
    <template #cell(irQuant)="{ item }">
      <div class="flex justify-center">
        <AppBadge :variant="Number(item.irQuant || 0) > 0 ? 'success' : 'neutral'" :text="formatQuantity(Number(item.irQuant || 0))" />
      </div>
    </template>
    <template #cell(iBronTask)="{ item }">
      <div class="flex justify-center">
        <AppBadge v-if="Number(item.iBronTask || 0) > 0" variant="warning" :text="formatQuantity(Number(item.iBronTask || 0))" />
        <AppTableCell v-else value="—" color="muted" align="center" />
      </div>
    </template>
    <template #cell(defectQuant)="{ item }">
      <div class="flex justify-center">
        <AppBadge :variant="Number(item.defectQuant || 0) > 0 ? 'error' : 'neutral'" :text="formatQuantity(Number(item.defectQuant || 0))" />
      </div>
    </template>

    <!-- Forward any other slots -->
...
const mergedColumns = computed(() => {
  const base = baseColumns.filter(c => !props.hideColumns.includes(String(c.key)))
  return [...base, ...props.extraColumns] as TableColumn<T>[]
})

defineExpose({
  triggerExcelExport: (name: string) => baseTableRef.value?.triggerExcelExport(name),
  setScrollTop: (val: number) => baseTableRef.value?.setScrollTop(val)
})
</script>
